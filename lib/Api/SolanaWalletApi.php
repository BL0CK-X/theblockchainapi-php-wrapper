...
class SolanaWalletApi
{
    ...
    public function solanaGenerateSecretRecoveryPhraseRequest()
    {
        ...
        $resourcePath = '/solana/wallet/secret_recovery_phrase';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
     ...
     * @return object[]
     */
    public function solanaGetTokensBelongingToWallet($network, $public_key)
    {
        list($response) = $this->solanaGetTokensBelongingToWalletWithHttpInfo($network, $public_key);
        return $response;
    }
     ...
     */
    public function solanaGetTokensBelongingToWalletWithHttpInfo($network, $public_key)
    {
        $request = $this->solanaGetTokensBelongingToWalletRequest($network, $public_key);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function solanaGetTokensBelongingToWalletAsync($network, $public_key)
    {
        return $this->solanaGetTokensBelongingToWalletAsyncWithHttpInfo($network, $public_key)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function solanaGetTokensBelongingToWalletAsyncWithHttpInfo($network, $public_key)
    {
        $returnType = 'object[]';
        $request = $this->solanaGetTokensBelongingToWalletRequest($network, $public_key);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function solanaGetTokensBelongingToWalletRequest($network, $public_key)
    {
        // verify the required parameter 'network' is set
        if ($network === null || (is_array($network) && count($network) === 0)) {
            throw new \InvalidArgumentException(
